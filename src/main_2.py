...
from sklearn.metrics import roc_auc_score, average_precision_score, confusion_matrix, f1_score, accuracy_score, roc_curve
...
def test(loader,data_type='test'):
    ...
    with torch.no_grad():
        ...
        return roc_auc_score(labels, scores), average_precision_score(labels, scores), loss_total/len(loader)


@torch.inference_mode()
def get_threshold(model: torch.nn.Module, loader: DataLoader) -> float:
    model.eval()
    best_threshold = 0
    for data in loader:
        data = data.to(device)
        out = model(data.x, data.edge_index, data.edge_mask, data.batch, data.z).view(-1)
        probs = out.sigmoid()

        # Get the best threshold from ROC Curve
        fpr, tpr, thresholds = roc_curve(data.label.view(-1,1).cpu().detach().numpy(), probs.cpu().detach().numpy())
        best_threshold += thresholds[np.argmax(tpr - fpr)]  # Maximize TPR - FPR

    return best_threshold/len(loader)
...
def evaluate_model(model: torch.nn.Module,
    ...
    for data in tqdm(loader, desc='Evaluate model'):  # Iterate in batches over the training/test dataset.
        data = data.to(device)
        out = model(data.x, data.edge_index, data.edge_mask, data.batch, data.z)
        loss = criterion(out.view(-1), data.label)
        out = out.cpu().clone().detach()
    ...
    labels = labels.cpu().clone().detach()
   
    probs = scores.sigmoid()
    y_pred = (probs > threshold).int()

    auc = roc_auc_score(labels.numpy(), scores.sigmoid().numpy())
    tn, fp, fn, tp = confusion_matrix(labels.numpy(), y_pred.numpy()).ravel().tolist()
    f1 = f1_score(labels.numpy(),  y_pred.numpy())
    acc = accuracy_score(labels.numpy(),  y_pred.numpy())

    tpr = tp/(tp + fn)
    fpr = fp/(fp + tn)

    return loss_total/len(loader), acc, auc, f1, tn, fp, fn, tp, tpr, fpr
...
best_threshold = get_threshold(model, val_loader)
...
test_tp,
test_tpr,
test_fpr) = evaluate_model(model, test_loader, criterion, best_threshold)
...
print(f"test AUC: {test_auc:.4f} | test f1: {test_f1:.4f} | test TPR: {test_tpr:.4f} | test FPR: {test_fpr:.4f}")
